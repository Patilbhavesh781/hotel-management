...
module.exports.otpVerify = async (req, res) => {
  try {
    ...
    const pending = await PendingSignup.findOne({ email: normalizedEmail, role: "host" });
    if (!pending) {
    ...
    if (await Host.findOne({ username: pending.username })) {
      await PendingSignup.deleteOne({ _id: pending._id });
      return res.json({ success: false, message: "Username already taken" });
    }

    const newHost = new Host({ email: normalizedEmail, username: pending.username });
    const registeredHost = await Host.register(newHost, pending.password);
    await PendingSignup.deleteOne({ _id: pending._id });
